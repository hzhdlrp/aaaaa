...
struct Fixed {
    ...
    static constexpr Fixed from_raw(int_N x) {
        Fixed ret;
        ret.v = x;
        return ret;
    }
...
template <typename int_N, int M>
Fixed<int_N, M> operator+(Fixed<int_N, M> a, Fixed<int_N, M> b) {
    return Fixed<int_N, M>::from_raw(a.v + b.v);
}
...
template <typename int_N, int M>
Fixed<int_N, M> operator*(Fixed<int_N, M> a, Fixed<int_N, M> b) {
    return Fixed<int_N, M>::from_raw(((int64_t) a.v * b.v) >> M);
}

template <typename int_N, int M>
Fixed<int_N, M> operator/(Fixed<int_N, M> a, Fixed<int_N, M> b) {
    return Fixed<int_N, M>::from_raw(((int64_t) a.v << 16) / b.v);
}
...
template <typename int_N, int M>
Fixed<int_N, M> operator-(Fixed<int_N, M> x) {
    return Fixed<int_N, M>::from_raw(-x.v);
}

template <typename int_N, int M>
Fixed<int_N, M> abs(Fixed<int_N, M> x) {
    if (x.v < 0) {
        x.v = -x.v;
    }
    return x;
...
template <typename int_N, int M>
ostream &operator<<(ostream &out, Fixed<int_N, M> x) {
...
void readField(char *path, FIXED &g_) {
    FILE *file = fopen(path, "r");
    int i = 0;
    while(fgets(field[N], M, file)){
        ++i;
        if (i == N) break;
    }
    float g, r1, r2;
    fscanf(file, " %f %f %f", &g, &r1, &r2);
    g_ = FIXED(g);
    rho[' '] = FIXED(r2);
    rho['.'] = FIXED (r1);
    fclose(file);
                ...
                for (auto [dx, dy] : deltas) {
                    ...
                    if (old_v > 0) {
                        ...
                        if (field[x + dx][y + dy] == '#') {
                            ...
                            p[x + dx][y + dy] += force / FIXED(dirs[x + dx][y + dy]);
